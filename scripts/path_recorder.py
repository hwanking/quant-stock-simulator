    ...
    with open(out_path, 'a', encoding='utf-8') as out:
        for i, tk in enumerate(tks):
            ...
            if not todo:
                continue
            try:
                # ⚠️ 라운드 333 — 여기는 **일봉만** 쓴다(고·저·종가·거래량). 종전엔 적재 함수가
                #   일봉 앞에 실시간 삼중 확인까지 불러 종목당 2.03초였고(2026-09-17 클라우드 실측 ·
                #   1,547종목 3,144초), 이 단계는 두 기록기를 이어 돌려 **90분 예산에서 잘렸다**
                #   (진입기준선 기록기가 1,040/1,547 에서 죽었다). 라운드 330 이 떼어 둔 일봉 전용
                #   함수로 바꾼다 — 같은 일봉이고(19/19 대조) 받는 초만 줄어든다.
                #   실패는 삼키되 **집계로는 남긴다**(옛 주석: 반환 튜플을 df 로 착각해 98종목이
                #   조용히 빠진 적이 있다 — 이제 df 하나를 받는다).
