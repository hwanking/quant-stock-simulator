...
■ 무엇을 판정하나
  ...
  · 밀린 양 > 허용치     → 실패
  · 밀린 양 < 0          → 실패 (원장이 **줄었다** — 라운드 245)
  · 0건을 봤다           → 실패 (미측정을 통과로 찍지 않는다)

■ 라운드 245 — 이 가드가 갈라짐을 **보고도 통과시켰다**
  위 설명은 *"원장은 늘기만 하므로 차이가 곧 밀린 양"* 을 전제한다.
  그 전제가 깨졌다. 클라우드가 08-19~09-01 연속 실패한 뒤 08-18 스냅샷
  에서 다시 시작해, 그 사이 손으로 키운 65,566행이 빠진 원장(185,962)을
  키우고 있었다. 산출물은 250,725줄일 때 만든 것이라 `lag = now - rows`
  가 **−64,763** 이었고 `lag <= MAX_LAG` 는 참이라 [OK] 가 찍혔다.
  2026-09-07 실행 로그에 그 줄이 그대로 남아 있다.
  음의 밀림은 '신선함'이 아니라 **원장이 줄었거나 갈라졌다**는 뜻이다.
...
def check(studies=STUDIES, max_lag=MAX_LAG):
    ...
    for rel, label in studies:
        ...
        if rows is None:
            ...
            continue
        lag = now - int(rows)
        # ⚠️ 라운드 245 — 음수는 통과가 아니다. 산출물이 **지금보다 큰**
        #   원장에서 만들어졌다는 뜻이고, 원장은 늘기만 한다는 전제가
        #   깨졌다는 뜻이다(실제로 클라우드가 작은 스냅샷에서 다시
        #   시작해 −64,763 이 찍혔는데 [OK] 였다).
        if lag < 0:
            print(f'  [FAIL] {label} — 원장 {int(rows):,}줄일 때 만들었는데 '
                  f'지금 원장은 {now:,}줄이다 (**{-lag:,}줄 줄었다**) · '
                  f'made={doc.get("made")}')
            bad.append(rel)
            continue
    ...
    if bad:
        print(f'\n>> 낡았거나 없다: {bad}')
        print('   자동으로 도는 연구가 멈춰 있거나, 원장이 줄었다는 뜻이다. '
              '워크플로가 어느 스냅샷에서 시작했는지 본다.')
        return 1
