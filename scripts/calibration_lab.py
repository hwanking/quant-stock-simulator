...
#:
#: ⚠️ 라운드 291 — **지금은 그 말이 전부 참이 아니다.** 위 진단용 8종목 중 **2개**가
#:   이 목록에도 있어(2026-09-14 실측) 그 둘의 케이스는 `cohort='holdout'` 으로 찍힌다
#:   — 산식을 손댄 종목이 '손대지 않은' 갈래 안에 있다. 267개 중 2개(0.7%)이고
#:   행으로도 작지만, **이름이 약속하는 것과 다르므로 여기 적는다**(§3).
#:   지우지 않는 이유: 이 목록을 바꾸면 다음 랩 실행이 원장을 통째로 다시 쓰면서
#:   두 종목의 케이스가 갈래를 옮겨 **연구 표본이 바뀐다**(R164 가 참았던 자리).
#:   홀드아웃으로 무언가를 판정하려는 라운드는 **그 둘을 빼고 세고 그 사실을 적는다.**
#:   회귀가 이 주석과 실제 겹침이 어긋나지 않는지 본다 — 겹침이 0 이 되면 이 문단을
#:   지우라고 실패한다(값을 잠그는 것이 아니라 **말과 사실을 묶는다** · R213).
HOLDOUT_TICKERS = [
    "000660.KS",  # SK하이닉스
    "005380.KS",  # 현대차
    "035420.KS",  # NAVER
