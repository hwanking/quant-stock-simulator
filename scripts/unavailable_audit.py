...
def clean_render_text(parts, skip_history=True):
    ...
    txt = re.sub(r'\{[^{}]*:[^{}]*\}', ' ', txt)
    # 라운드 331 — 인라인 마크다운 표시(`**` 굵게 · 백틱 코드)를 걷는다. 화면에는 안 보이는 글자인데
    #   값을 감싸면(예: 굵은 코드 글씨의 '산출 불가' 뒤에 ' — 20일 유효표본 2건') 낱말과 사유 사이에 끼어
    #   **사유가 같은 줄에 붙어 있는데도** '사유 없음'으로 셌다(라운드 318 이 옮겨 둔 그 줄 · 유효표본이
    #   5건 아래로 내려간 날 처음 드러남). 판별 규칙은 그대로다 — 사유는 여전히 낱말 **바로 뒤** 구분자로 시작한다.
    txt = re.sub(r'\*\*|`', '', txt)
    return re.sub(r'[ \t]+', ' ', txt)
