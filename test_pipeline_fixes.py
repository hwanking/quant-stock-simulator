...
def _intent144(q):
    """answer() 와 **같은 길**로 잰다 — 정규화 후 대조 (라운드 91).

    ⚠️ 처음엔 원문으로 대조하는 판을 따로 썼다. 그러면 실제 코드가
    정규화를 해도 검사는 옛 경로를 재게 된다 — 검사와 코드가 다른 길을
    가면 통과가 아무것도 보장하지 않는다.
    """
    qn = _gc144._norm(q)
    for name, words in _gc144._INTENTS:
        if any(_gc144._norm(w) in qn for w in words):
            return name
    return None
