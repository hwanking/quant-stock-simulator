...
if _os.path.exists(_cal55):
    ...
    # ══════════════════════════════════════════════════════════════
    # 라운드 195 — 사전등록을 실행하고, **결과대로** 계약을 다시 썼다
    #   (docs/PREREG_R192_BAND_MONOTONE.md · scripts/band_monotone_r195.py)
    #
    #   ■ 왜 다시 쓰나
    #     이 두 계약은 (50,59)·(60,100) 칸을 **손으로 물어** 오래 죽어
    #     있었다. 랩이 칸을 더 잘게 갈랐는데(50-54·55-59 / 60-64·65-69·
    #     70-100) 검사는 옛 이름을 물었고, `.get()` 이 `{}` 를 주니 n 이
    #     0 이 되어 **표본 미달처럼 보였다**(라운드 192 가 그 사유 문구를
    #     고쳤고, 라운드 195 가 계약 자체를 되살린다).
    #     → 이제 **밴드를 산출물에서 유도한다.** 키를 손으로 묻지 않는다.
    #
    #   ■ 재고 나서 안 것 (2026-08-31 · 원장 176,942건 기준 산출물)
    #     R1 검사 자신의 표본 규칙을 글자 그대로 옮김 (아래 n>=30 ·
    #        맨 위 칸 n>=15) — 통과
    #     R2 살아남는 칸 7개 (>=3) — 통과
    #     R3 **점추정 단조성은 깨진다** — 57.14 < 57.25 < 59.26 < 59.67
    #        < 59.99 < 60.21 … 그러고 나서 **(70,100) 이 55.00%**
    #     R4 그 끊김은 **잡음과 구분되지 않는다** — Wilson 구간이
    #        (65,69) [56.13, 64.16] · (70,100) [34.21, 74.18] 로 겹친다
    #        (n=20)
    #
    #   ■ 그래서 무엇을 계약으로 삼는가
    #     사전등록이 정한 두 길 중 ① 을 택한다 — *"비단조를 사실로 적고
    #     계약을 그 사실에 맞게 다시 쓴다."*
    #       · 실패로 세는 것은 **증명된 역전**뿐이다 — 아래 칸의 Wilson
    #         하한이 위 칸의 상한을 넘는 경우. **새 문턱을 만들지 않는다**
    #         (z=1.96 은 §2-4 가 이미 채택한 잣대다).
    #       · 점추정 비단조는 **매 실행 detail 에 찍는다.** 숨기지 않는다.
    #     이것이 약한 계약이라는 것도 적어 둔다 — 큰 역전만 잡는다.
    #     라운드 110 이 *"순위에 정보가 없다"* 를 이미 냈으므로, 이 계약은
    #     *"점수가 확률을 가른다"* 를 **증명하지 않는다.**
    # ══════════════════════════════════════════════════════════════
    import math as _math55

    def _wilson55(k, n, z=1.96):
        if not n:
            return (0.0, 100.0)
        p = k / n
        d = 1.0 + z * z / n
        c = p + z * z / (2 * n)
        r = z * _math55.sqrt(p * (1 - p) / n + z * z / (4 * n * n))
        return ((c - r) / d * 100.0, (c + r) / d * 100.0)

    _bl55 = sorted((b for b in (_c55.get('bands') or [])
                    if b.get('n') is not None and b.get('hit_rate') is not None),
                   key=lambda b: (b['lo'], b['hi']))
    # 검사 자신의 표본 규칙 — 글자 그대로 (아래 30 · 맨 위 15)
    _kept55 = [b for _i55, b in enumerate(_bl55)
               if b['n'] >= (15 if _i55 == len(_bl55) - 1 else 30)]
    check("점수 구간을 산출물에서 유도한다 (옛 키를 손으로 묻지 않는다)",
          len(_kept55) >= 3,
          ' · '.join(f"({b['lo']},{b['hi']}):n={b['n']}" for b in _kept55),
          scanned=len(_bl55))

    _pt55, _proven55 = [], []
    for _i55 in range(len(_kept55) - 1):
        _a55, _b55 = _kept55[_i55], _kept55[_i55 + 1]
        if _a55['hit_rate'] < _b55['hit_rate']:
            continue
        _pt55.append(f"({_a55['lo']},{_a55['hi']}) {_a55['hit_rate']:.2f}%"
                     f" >= ({_b55['lo']},{_b55['hi']}) {_b55['hit_rate']:.2f}%")
        _wa55 = _wilson55(round(_a55['hit_rate'] / 100 * _a55['n']), _a55['n'])
        _wb55 = _wilson55(round(_b55['hit_rate'] / 100 * _b55['n']), _b55['n'])
        if _wa55[0] > _wb55[1]:                # 구간이 안 겹친다 = 증명된 역전
            _proven55.append(
                f"({_a55['lo']},{_a55['hi']})↓{_wa55[0]:.1f}% > "
                f"({_b55['lo']},{_b55['hi']})↑{_wb55[1]:.1f}%")
    check("점수 구간 적중률에 **증명된 역전**이 없다 (Wilson · 새 문턱 없음)",
          not _proven55,
          (f"점추정 비단조 {len(_pt55)}곳: {'; '.join(_pt55)}"
           if _pt55 else '점추정도 단조')
          + (f" · 증명된 역전 {_proven55}" if _proven55 else ' · 증명된 역전 없음'),
          scanned=len(_kept55))

    # ── 두 번째 계약 — 재 보니 **거짓 주장이었다** ────────────────────
    #   종전: check("매수권(60점+) 평균수익 양수 — 비용 차감 후에도", …)
    #   실측(같은 산출물): 60점+ 세 칸 합산 n=13,011 · 적중 59.99% ·
    #   평균수익 +0.141% · 비용 0.55 차감 후 **−0.409%** — 음수다.
    #   이 저장소가 이미 적어 둔 결론(§9 · 라운드 159·168)과 같다.
    #   **그러므로 그 계약은 되살리지 않는다** — 되살리면 회귀가 사실이
    #   아닌 것을 요구하게 된다. 대신 **숨기지 않는가**를 계약으로 삼는다.
    _top55 = [b for b in _bl55 if b['lo'] >= 60]
    _n55 = sum(b['n'] for b in _top55)
    _ev55 = (sum(b['n'] * (b.get('avg_return') or 0.0) for b in _top55) / _n55
             if _n55 else None)
    _net55 = (None if _ev55 is None else _ev55 - 0.55)
    check("매수권(60점+) 비용 차감 기대값을 값으로 재 둔다",
          _net55 is not None,
          f"n={_n55:,} · 평균 {_ev55:+.3f}% · 비용 0.55 차감 후 "
          f"{_net55:+.3f}%" if _net55 is not None else '산출 불가',
          scanned=len(_top55))
    if _net55 is not None and _net55 <= 0:
        check("우위가 없으면 화면이 없다고 적는다 (§9)",
...
# ⚠️ 라운드 142 — 이 검사는 버튼 **문구**('이 목록을 관심종목에 더하기')를
#   붙잡고 있었다. 문구가 '점수 나온 N종목을 관심종목에 더하기' 로 바뀌며
#   실패했는데, **재려던 성질은 그대로다** — 덮어쓰기가 아니라 더하기.
#   그러니 문구가 아니라 **성질**을 본다: `_wl_add`(합치기)를 부르고,
#   목록을 통째로 갈아끼우지 않는다.
#   (이 저장소가 반복해서 고친 그 부류다 — 검사를 자리에 못 박지 않는다)
check("스캔 결과 저장이 **덮어쓰기가 아니라 더하기**다",
      '관심종목에 더하기' in _w179
      and '_wl_add(_r[' in _w179
      and "st.session_state['watchlist'] = _items" not in _w179,
      '목록을 통째로 갈아끼우면 공들여 담은 것이 사라진다')
...
# ══════════════════════════════════════════════════════════════════════
# §181 — 라운드 137: 긁기 전에 폭을 쟀는가
#
#   라운드 134 가 다음 걸음을 "그날 수급 붙은 종목 수를 늘려라. 다만
#   **늘어나는 폭을 먼저 재고 시작한다**" 고 적었다. 전 종목 수집은
#   약 24만 요청 · 40시간이라 공짜가 아니다.
#
#   재 보니 종목당 **+0.02일**이었다 — 첫 50종목이 373일을 벌고 나머지
#   200종목이 20일을 벌었다. P4 하한 600일에 닿으려면 시장 전체(약
#   2,700종목)의 **네 배**가 필요하다. 비싼 것이 아니라 **길이 없다.**
#
#   이 절이 지키는 것:
#     ⓐ 결정 전에 **쟀는가** (곡선이 산출물에 있는가)
#     ⓑ 포화가 **말이 아니라 곡선의 값**에서 나오는가
#     ⓒ 외삽을 **가정이라고 밝혔는가** (지어낸 값을 사실처럼 쓰지 않는다)
#     ⓓ 결정을 적었는가 — 안 한다고 정했으면 그렇게
# ══════════════════════════════════════════════════════════════════════
print("\n" + "=" * 72)
print("§181 라운드 137 수급 종목 곡선 — 긁기 전에 폭을 쟀는가")
print("=" * 72)
_p181 = _os.path.join(PROJ, 'data', 'flow_ticker_curve_r137.json')
_doc181p = _os.path.join(PROJ, 'docs', 'RESULT_R137_FLOW_TICKER_CURVE.md')
if not _os.path.exists(_p181):
    check("R137 산출물이 있다", False, 'data/flow_ticker_curve_r137.json 없음')
else:
    with open(_p181, encoding='utf-8') as _f181:
        _d181 = _json.load(_f181)
    _doc181 = _read148(_doc181p)
    _cv181 = _d181.get('curve') or []

    # ⓐ 쟀는가 — 곡선이 여러 점으로 있는가
    check("곡선이 5점 이상이다", len(_cv181) >= 5, f'{len(_cv181)}점')
    check("종목 수가 실제로 늘어나는 곡선이다",
          all(_cv181[i]['tickers'] < _cv181[i + 1]['tickers']
              for i in range(len(_cv181) - 1)),
          str([c['tickers'] for c in _cv181]))

    # ⓑ 포화가 **값**에서 나오는가 — 첫 구간이 나머지 전부보다 크다
    if len(_cv181) >= 5:
        _first181 = _cv181[0]['paired_days']
        _rest181 = _cv181[-1]['paired_days'] - _cv181[0]['paired_days']
        check("첫 구간이 나머지 전부보다 크다 (포화의 정의)",
              _first181 > _rest181 * 5,
              f'첫 {_first181}일 vs 나머지 {_rest181}일')
        # 케이스는 늘었는데 날짜는 안 늘었다 — 이 라운드의 핵심
        _cx181 = _cv181[-1]['cases'] / max(1, _cv181[0]['cases'])
        _dx181 = _cv181[-1]['paired_days'] / max(1, _cv181[0]['paired_days'])
        check("케이스 증가율이 날짜 증가율보다 훨씬 크다 (케이스≠표본)",
              _cx181 > _dx181 * 3,
              f'케이스 {_cx181:.1f}배 vs 날짜 {_dx181:.2f}배')

    # ⓒ 외삽을 가정이라고 밝혔는가
    check("외삽임을 문서가 밝힌다",
          '외삽' in _doc181 and '가정' in _doc181)
    check("외삽이 낙관적인 쪽임을 밝힌다",
          '이보다 덜 는다' in _doc181)
    check("산출물도 방법을 적는다",
          '지어낼 수 없으므로' in str(_d181.get('method')))

    # ⓓ 결정을 적었는가
    check("늘리지 않기로 한 결정을 적었다",
          '늘리지 않는다' in _doc181 and '수급 종목 확대를 하지 않는다'
          in _doc181)
    check("왜 불가능한지 자릿수로 적었다 (시장 전체의 네 배)",
          '2,700' in _doc181 and '네 배' in _doc181)
    check("이미 모은 것은 버리지 않는다고 적었다",
          '그대로 둔다' in _doc181)
    # 같은 모양이 세 번째임을 잇는다 (계보)
    check("R129·R134 와 같은 모양임을 잇는다",
          '129' in _doc181 and '134' in _doc181
          and '케이스는 늘고 날짜는 안 는다' in _doc181)
    # 측정 전용 — 운영 미변경
    check("측정 전용임을 못 박았다",
          '측정 전용' in _doc181
          and '바꾸지 않는다' in str(_d181.get('note')))
    _s181 = _read148(_os.path.join(PROJ, 'scripts',
                                   'flow_ticker_curve_r137.py'))
    check("측정 코드도 R131 함수를 재사용한다 (다시 구현하지 않았다)",
          'import flow_rank_r131 as R' in _s181
...
# ══════════════════════════════════════════════════════════════════════
# §185 — 관심종목: 담는 대상·담는 자리·값 갱신 (라운드 142)
#
#   사용자가 셋을 지적했다:
#     ① "더하기 하면 왜 5개가 다 들어가나? 다음 거래일에 실제로 손댈 수
#        있는 후보만 들어가야 하는 것 아닌가" — **맞다.** 바로 위에서
#        `_scored_rows`/`_excluded_rows` 로 갈라 놓고도 담을 때는
#        `_att['rows']` 전체를 썼다. 제외된 종목까지 들어갔다.
#     ② "검색하는 종목에 관심추가 버튼도" — 버튼은 라운드 135 부터
#        있었지만 **접힌 칸 안**이라 검색 직후 안 보였다. 라운드 136 의
#        "관심목록 어디서 봐?" 와 같은 모양이다.
#     ③ "값을 주기적으로 저장" — 라운드 141 은 **그 종목을 볼 때만**
#        찍어서, 안 열어 본 종목은 영원히 비어 있었다.
# ══════════════════════════════════════════════════════════════════════
print("\n" + "=" * 72)
print("§185 관심종목 — 담는 대상·자리·값 갱신 (라운드 142)")
print("=" * 72)
_w185 = _read148(_os.path.join(PROJ, 'web_app.py'))

# ① 점수가 나온 종목만 담는다 — 제외 종목이 섞이면 안 된다
check("더하기가 **점수 나온 것만** 담는다",
      'for _r in _scored_rows:' in _w185
      and "for _r in _att['rows']:" not in _w185,
      "_att['rows'] 전체를 담으면 제외 종목까지 들어간다")
check("버튼 이름이 몇 종목인지 말한다",
      "f\"점수 나온 {len(_scored_rows)}종목을 \"" in _w185)
check("담을 것이 없으면 버튼을 잠근다",
      'disabled=not _scored_rows' in _w185)
check("안 담은 종목을 **조용히 거르지 않는다** (§3)",
      '담지 않았습니다' in _w185 and '_excluded_rows[:4]' in _w185)

# ② 검색 직후 보이는가 — 접힌 칸이면 없는 것과 같다
check("안 담긴 종목이면 사이드바 칸이 **펼쳐진다**",
      '_wl_cur_open = ' in _w185 and 'expanded=_wl_cur_open' in _w185)
check("본문 관심종목 절에도 담기 버튼이 있다",
      "key='wlb_add_cur'" in _w185 and '지금 보는 종목 담기' in _w185)
check("이미 담긴 종목이면 그렇게 말한다",
      '이미 담겨 ' in _w185)

# ③ 값이 주기적으로 갱신되는가 — 안 열어 본 종목도 채워져야 한다
check("개장 전 리포트에서도 관심종목 값을 채운다",
      '_wl_codes142 = ' in _w185 and '_by142' in _w185)
check("갱신 값이 리포트의 CORE 에서 온다 (§4)",
      "_co142.get('pullback_zone')" in _w185
      and "_co142.get('new_target')" in _w185)
check("2차는 보유자 기준값을 쓴다 (기준을 섞지 않는다)",
      "'snap_t2': _pk142.get('target2')" in _w185
      and '현재가 기준' in _w185)
check("못 낸 값으로 옛 값을 덮어쓰지 않는다 (갱신 경로에서도)",
      _w185.count('덮어쓰지 않는다') >= 2)
check("리포트 기준일을 그대로 쓴다 (오늘 값인 척 안 한다)",
      "_pm_today.get('data_asof')" in _w185 and "'snap_at': _asof142" in _w185)
check("관심종목 갱신 때문에 개장 전 화면이 죽지 않는다",
      '관심종목 갱신 때문에 개장 전 화면이 죽지 않는다' in _w185)

# 두 갱신 경로가 **같은 키**를 쓰는가 — 다르면 화면이 갈린다 (§4)
for _k185 in ('snap_buy', 'snap_t1', 'snap_t2', 'snap_fair',
              'snap_at', 'snap_engine'):
    check(f"두 갱신 경로가 같은 키를 쓴다 — {_k185}",
...
if not _os.path.exists(_p186):
    ...
    # ⓓ 판정 — §8(부분 표본)의 문구는 **역사로 남는다.** 완주(§9)가
    #    크기를 수정했지만 §8 을 지우지 않는다 — 두 결과를 나란히 싣는
    #    것이 이 저장소 방식이다(R134 가 R131 을 대체하지 않은 것과 같다).
    check("부분 표본 시점의 판정이 역사로 남아 있다 (§8)",
          '시대 편향은 있지만 작다' in _flat186
          and '9년 내내 비슷하게 먹힌다' in _flat186)
    # 라운드 145 — 완주가 판정의 크기를 키웠다. 숨기지 않는다.
    check("§8 이 부분 표본 기준임을 §8 스스로 밝힌다",
          '이 절은 부분 표본' in _flat186)
    check("완주가 크기를 수정한 경위를 적었다 (3.4%p → 5.5%p)",
          '진짜 옛 구간을 못 봤다' in _flat186
          and '완주하니 크기가 커졌다' in _flat186)
    check("표본 확장이 결론을 수정한 세 번째임을 이었다",
          '세 번째' in _flat186 and '크기가 매번 수정됐다' in _flat186)
    check("이벤트 연구 함의를 적었다 (차이를 사전등록에서 다룬다)",
          '이 차이를 명시적으로 다뤄야 한다' in _flat186)
...
# §6 — 라운드 223 이 이 검사를 현실에 맞췄다. 종전엔 블라인드 적중 '50.4%' 를
#   소스 리터럴로 요구했는데, R223 이 그 수(와 잰 날짜·당시 행수)를 R159 의
#   산출물(`data/census_r159.json`)에서 읽게 했다 — 손으로 적은 숫자를 없애고
#   R78 의 옛 재평가일 잠금도 피하려고. 잠글 것은 셋: 문장이 우위 없음을 말한다 ·
#   수는 산출물에서 온다 · 그 산출물(판정한 날 그대로 · R107)이 실제로 50.4 를 든다.
import json as _json204b
with open(_os.path.join(PROJ, 'data', 'census_r159.json'), encoding='utf-8') as _f204b:
    _c159_204 = _json204b.load(_f204b)
check("우위가 없다는 사실을 견해에 함께 적는다 (§9) — 수는 R159 산출물에서 읽는다",
      '재현되는 우위가 확인되지 않았습니다' in _wa204
      and "_c159_be.get('blind_hit_pct')" in _wa204
      and float((_c159_204.get('breakeven') or {}).get('blind_hit_pct') or 0) == 50.4
      and float((_c159_204.get('breakeven') or {}).get('need_hit_pct') or 0) == 63.7)
...
# ⚠️ 라운드 178 — 이 검사가 `'fair_ref * 1.15' in …` 라는 **글자**를 찾고
#   있었는데, 구역 판정을 `_zone_of()` 로 묶으면서 그 표기가 `_fair * 1.15`
#   로 바뀌어 실패했다. **문턱은 한 글자도 안 바뀌었다** — 검사가 옛 표기를
#   요구하고 있었을 뿐이다(§6). 표기가 아니라 **값**으로 확인하게 고친다:
#   구역 함수를 실제로 돌려 1.15 가 경계인지 본다.
_z211 = None
for _n211f in _ast201.walk(_ast201.parse(_qi211)):
    if (isinstance(_n211f, _ast201.FunctionDef)
            and _n211f.name == '_zone_of'):
        _z211 = _ast201.get_source_segment(_qi211, _n211f)
_zns211 = {}
if _z211:
    exec(_z211, _zns211)                                         # noqa: S102
check("구역 판정 함수를 떼어 냈다", '_zone_of' in _zns211)
# ⚠️ 라운드 183 — 이 검사가 **옛 설계(15%)를 요구**해 실패했다.
#   가드를 +15% 에서 **0%** 로 당겼기 때문이다(블라인드 실측 근거는
#   docs/RESULT_R183_VALUE_GATE.md). **지키려는 성질은 그대로다** —
#   *"문턱을 새로 지어내지 않고 구역 판정의 경계를 재사용한다."*
#   경계만 1.15 → 1.00 으로 현실에 맞춘다 (§6).
check("문턱을 새로 고르지 않았다 (가드 0% = 구역 판정 1.00배)",
      '_OVER_BLOCK_PCT = 0.0' in _na211
      and _zns211['_zone_of'](100.0, 100.0)[0] == '적정가 이하 (안전마진 미확보)'
      and _zns211['_zone_of'](100.1, 100.0)[0] != '적정가 이하 (안전마진 미확보)',
      '가드의 0% 와 구역 판정의 ×1.00 이 같은 자리를 가리켜야 한다')
# 그리고 **그 경계가 임의로 옮겨지지 않았는지** — 구역 문턱 자체는 그대로다
check("구역 판정의 문턱(1.05·1.15)은 안 건드렸다",
      _zns211['_zone_of'](104.9, 100.0)[0] == '적정가 소폭 초과'
      and _zns211['_zone_of'](115.1, 100.0)[0] == '적정가 크게 초과 (추격매수 위험)',
      '가드를 당긴 것이지 구역 정의를 바꾼 것이 아니다')
...
# ⚠️ 라운드 183 — 문턱을 **+15% 에서 0% 로 당겼다.** 블라인드 실측에서
#   적정가를 넘는 순간 음수였다(적정가 이하 +0.238 · 소폭 초과 −0.890 ·
#   경고 −0.652). 새 숫자를 만들지 않았다 — 구역 판정의 경계(×1.00)다.
#   ⚠️ 표본이 작다(71·97). 그 사실은 문서와 코드 주석에 적었다.
check("적정가를 넘는 순간 막는다 (+0.1%)", _kind211(0.1) == 'observe',
      _kind211(0.1))
check("적정가 이하는 그대로 둔다 (−0.1%)", _kind211(-0.1) != 'observe',
      _kind211(-0.1))
check("경계(0.0%)는 통과 — 넘어야 막는다", _kind211(0.0) != 'observe',
      _kind211(0.0))
check("문턱을 새로 지어내지 않았다 (구역 경계 ×1.00 재사용)",
      '_OVER_BLOCK_PCT = 0.0' in _na211
      and _zns211['_zone_of'](100.1, 100.0)[0] != '적정가 이하 (안전마진 미확보)',
      '적정가 × 1.00 이 구역 판정의 경계와 같아야 한다')
# ⚠️ 표본 표기는 **주석**에 있다. `code_lines` 는 산 코드만 보므로
#   (라운드 71) 여기서는 **원문**을 읽어야 한다 — 처음에 code_lines 로
#   찾다가 헛짚었다.
_naraw183 = _read148(_os.path.join(PROJ, 'next_action.py'))
check("표본이 작다는 사실을 코드 주석에 적었다",
      '71·97' in _naraw183 or 'n  71' in _naraw183 or 'n 71' in _naraw183,
      '표본을 숨기고 문턱만 옮기면 라운드 27b 와 같은 실수다')
...
# ══════════════════════════════════════════════════════════════════════
# §212 — 아는 값을 두고 '판정 불가'라고 적고 있었다 (라운드 178)
#
#   라운드 177 이 **차단**은 고쳤는데(`fair_overshoot_pct`) **라벨**은
#   그대로였다. 화면 census 로 훑으니 현대건설 한 종목에서
#   `진입 위치 [판정 불가] → 상한 59점` 이 **여섯 자리**에 나왔다 —
#   바로 옆에서 `적정가 대비 상승여력 −24.8%` 를 적어 놓고서.
#   **아는 값을 두고 모른다고 적는 것**이라 §3 위반이다.
#
#   그런데 라벨을 고치면 `price_location_score` 도 움직인다. 점수를
#   **올리는** 쪽은 측정 없이 완화하는 것이라 §2 위반이다. 그래서
#   **라벨은 정직하게, 점수는 보수적인 쪽으로만**:
#   권장 매수가가 없을 때는 종전 30 을 **상한**으로 두고 더 낮을 때만 내린다.
#     크게 초과 10 · 경고 22 → 내림   |   소폭 초과 40 · 적정가 이하 60 → 30 유지
#   **올라가는 칸이 하나도 없다** — 그래서 새 문턱을 만든 것이 아니다.
#
#   곁들여 '미수신'과 '기사 없음'을 섞어 쓰던 자리도 갈랐다 —
#   §3 이 *"데이터 미수신 ≠ 추천 없음, 두 문장을 반드시 분리한다"* 고
#   못 박은 그 자리다.
# ══════════════════════════════════════════════════════════════════════
print("\n" + "=" * 72)
print("§212 아는 값을 '판정 불가'로 적지 않는다 (라운드 178)")
print("=" * 72)

_qi212 = '\n'.join(ln for _i, ln in _la135.code_lines('quant_indicators.py'))
check("권장 매수가가 없어도 적정가를 들고 있는다", 'fair_ref_any' in _qi212)
check("적정가조차 없을 때만 '판정 불가'다",
      'buy_entry_max is None and fair_ref_any is None' in _qi212)
check("점수 상한을 종전 값으로 둔다 (완화 금지)",
      '_NO_RBP_SCORE_CAP = 30' in _qi212
      and 'min(_NO_RBP_SCORE_CAP, _z)' in _qi212)

# 구역 함수를 **실제로 떼어 돌린다** — 논리를 베끼면 아무것도 증명 못 한다
_zsrc212 = None
for _n212 in _ast201.walk(_ast201.parse(_qi212)):
    if isinstance(_n212, _ast201.FunctionDef) and _n212.name == '_zone_of':
        _zsrc212 = _ast201.get_source_segment(_qi212, _n212)
check("구역 함수를 코드에서 떼어 냈다", bool(_zsrc212))
_ns212 = {}
exec(_zsrc212, _ns212)                                           # noqa: S102
_zone212 = _ns212['_zone_of']

check("문턱이 종전 그대로다 (1.05 · 1.15)",
      _zone212(104.9, 100.0)[0] == '적정가 소폭 초과'
      and _zone212(114.9, 100.0)[0] == '적정가 초과 (추격매수 경고)'
      and _zone212(115.1, 100.0)[0] == '적정가 크게 초과 (추격매수 위험)')

# ⚠️ 핵심 — **어떤 위치에서도 점수가 30 을 넘지 않는다** (완화 0칸)
_CAP212 = 30
_raised212 = [r for r in (0.5, 0.8, 1.0, 1.03, 1.049, 1.05, 1.10, 1.149,
                          1.15, 1.16, 1.33, 1.5, 3.0)
              if min(_CAP212, _zone212(100.0 * r, 100.0)[1]) > _CAP212]
check("권장 매수가가 없을 때 점수가 올라가는 칸이 하나도 없다",
      not _raised212, f'올라간 칸 {_raised212}')
# 그리고 **실제로 내려가는 칸이 있다** — 없으면 이 변경이 아무 일도 안 한 것
_lowered212 = [r for r in (1.16, 1.33, 1.5)
               if min(_CAP212, _zone212(100.0 * r, 100.0)[1]) < _CAP212]
check("고평가 구간에서는 실제로 내려간다 (0칸이면 미측정)",
      len(_lowered212) >= 2, f'내려간 칸 {_lowered212}')

# ── '미수신'과 '기사 없음'을 섞지 않는다 (§3) ────────────────────────
_w212 = '\n'.join(ln for _i, ln in _la135.code_lines('web_app.py'))
check("뉴스 상태를 수집 실패와 기사 0건으로 가른다",
      "_news60 is None" in _w212 and "'기사 없음'" in _w212)
check("'미수신 · 수신 기사 없음' 이 한 줄에 같이 나오지 않는다",
      "'수신 기사 없음 — 판단에 미반영'" not in _w212,
      '앞은 우리 문제, 뒤는 정상 — 섞으면 안 된다 (§3)')

# ── 라운드 179 — 같은 모양이 적정가 카드에 하나 더 남아 있었다 ────────
#   장기 가치 참고선이 없다고 **적정가 대비 위치까지** 포기하고
#   '판정 불가'를 적었다. 바로 위 줄에서 `상승여력 -24.8%` 를 적어 놓고서.
check("참고선이 없어도 적정가 대비 위치는 말한다",
      "f\"참고선 미산출 · 적정가 대비 {upside_pct:+.1f}%\"" in _w212)
check("적정가마저 없을 때만 '판정 불가'라 적는다",
      '판정 불가 (적정가·참고선 모두 미산출)' in _w212)
check("그 자리도 한국 관행 색을 쓴다 (§5)",
      "_TOK['up'] if upside_pct > 0" in _w212)
# 옛 표기가 되살아나지 않게 — 맨 앞이 '판정 불가'인 채 끝나면 안 된다
check("맨 앞이 그냥 '판정 불가' 인 옛 문구는 사라졌다",
      'value_eval_str = "판정 불가"' not in _w212,
      '사유 없이 판정 불가만 적으면 아는 값을 버리는 것이다 (§3)')

# ── 라운드 180 — 업데이트 이력이 **현재 결함 목록처럼** 읽혔다 ────────
#   사용자가 목록의 한 줄을 지금 고장난 것으로 읽고 고쳐 달라고 했다.
#   그 줄은 **이미 고친 것의 제목**이다 — 커밋 규약상 제목이 '고치기 전의
#   문제'를 적기 때문이다. 제목은 이력 원문이라 못 바꾼다.
#   → **무엇을 읽고 있는지**를 목록 앞에 먼저 밝힌다.
check("업데이트 목록이 '이미 반영이 끝난 변경'임을 밝힌다",
      _w212.count('이미 반영이 끝난 변경') >= 2,
      '접힌 목록과 전체 목록 둘 다에 적어야 한다')
...
# ⚠️ 라운드 183 — 여기서 **판정이 뒤집혔다.** 라운드 182 는 §3 을 들어
#   고지만 붙였는데, 원장을 구역별로 재 보니 블라인드에서 '판정 불가'가
#   **가장 나빴다**(n 1,536 · 적중 52.9% · 손익분기 −10.0%p · 수익 −1.237%).
#   그래서 **모델이 거부한 경우(OUT_OF_DOMAIN)는 후보에서 뺀다.**
#   못 잰 경우(UNCALCULATED 등)는 §3 대로 그대로 둔다.
check("모델이 거부한 종목은 후보에서 뺀다 (블라인드 최악 구역)",
      _r182.get('kind') == 'observe', str(_r182.get('kind')))
check("그 사유를 숫자가 아니라 말로 밝힌다",
      '적정가 산출 불가' in str(_r182.get('exclude_reason') or ''),
      str(_r182.get('exclude_reason'))[:60])
# §3 — **못 잰 것과 모델 거부를 가른다.** 미수신까지 막으면 라운드 37 이다.
_r183u = _na211m.build(
    dict(_fs182, fair_value_status='UNCALCULATED'), _tech211, 41350.0) or {}
check("못 잰 것(UNCALCULATED)은 후보에서 빼지 않는다 (§3)",
      _r183u.get('kind') != 'observe', str(_r183u.get('kind')))
check("그 둘이 실제로 다르게 나온다 (같으면 안 가른 것)",
      _r182.get('kind') != _r183u.get('kind'),
      f"{_r182.get('kind')} vs {_r183u.get('kind')}")
...
# ══════════════════════════════════════════════════════════════════════
# §217 — 관점 점수를 매매 지시처럼 적지 않는다 (라운드 187 · R184 분석 P1)
#
#   사용자 분석 세 건을 함께 닫는다.
#     ⓐ `매수  78점 · 이 탭 단독 판정` — 탭 하나(자기유사)만 본 점수인데
#        행동어로 크게 찍혀 운영 판정처럼 읽혔다. 중앙 판정이 '추천 제외'
#        인 종목에서도 이 칸은 '매수'였다 (§4 — 화면이 두 말을 했다).
#     ⓑ `오늘의 AI 퀀트 최적 종목 TOP 3` — '최적'은 전 종목이 아니라
#        상위 몇 개만 정밀분석한 결과이고, 'TOP'(순위)은 라운드 110 이
#        원장 166,132건으로 재서 **차이 중앙값 0.0%p · z −1.39**(유의하지
#        않음)로 발표한 것이다. 우리가 없다고 적은 것을 제목이 팔았다 (§9).
#     ⓒ `변동성 관리 비중 제안: 자본의 20% 이내` — 추천하지 않는 종목의
#        결론 바로 아래에 붙어 매수 지시로 읽혔다.
#
#   고침은 셋 다 **표시 계층**이다. 문턱·점수·가격은 한 글자도 안 바꿨다.
#   ⓒ 는 새 조건을 만들지 않고 중앙 판정의 `recommended` 를 재사용한다
#   (R186 과 같은 자 · §2-6), 그리고 감출 때 **왜 안 내는지 적는다** (§3).
# ══════════════════════════════════════════════════════════════════════
print("\n" + "=" * 72)
print("§217 관점 점수는 매매 지시가 아니다 (라운드 187)")
print("=" * 72)

_w217 = '\n'.join(ln for _i217, ln in _la135.code_lines('web_app.py'))
_qi217 = '\n'.join(ln for _i217, ln in _la135.code_lines('quant_indicators.py'))

# ── ⓐ 탭 판정어 — 함수를 실제로 돌린다 (논리를 베끼지 않는다) ────────
_vf217 = qi.QuantIndicatorsEngine._verdict_from_score
_words217 = {s: _vf217(s)[0] for s in (95, 72, 71, 60, 59, 45, 44, 32, 31, 0)}
check("탭 판정어에 행동어(매수/매도)가 없다",
      not any(('매수' in v) or ('매도' in v) for v in _words217.values()),
      str(sorted(set(_words217.values()))))
check("우호도 다섯 단계가 다 나온다",
      len(set(_words217.values())) == 5, str(sorted(set(_words217.values()))))
check("산출 불가는 그대로다", _vf217(None)[0] == '산출 불가')
# **문턱과 색은 안 바꿨다** — 낱말만 바꿨다는 증거 (안 넣으면 완화가 섞인다)
check("문턱이 종전 그대로다 (72·60·45·32 경계에서 갈린다)",
      _vf217(72)[0] != _vf217(71)[0] and _vf217(60)[0] != _vf217(59)[0]
      and _vf217(45)[0] != _vf217(44)[0] and _vf217(32)[0] != _vf217(31)[0])
check("색이 종전 그대로다",
      [_vf217(s)[1] for s in (95, 71, 59, 44, 0)]
      == ['#30d158', '#7bd88f', '#ff9f0a', '#ff7a45', '#ff453a'])
check("화면이 '이 탭 단독 판정'이라 적지 않는다",
      '이 탭 단독 판정' not in _w217)
check("어느 관점인지와 매매 판정이 아님을 적는다",
      '매매 판정 아님' in _w217 and "t.get('label')" in _w217)
# 탭 판정은 **표시 전용**이다 — 점수 계산에 이 낱말이 들어가면 안 된다
check("탭 판정어가 점수 계산에 쓰이지 않는다",
      "t['verdict']" not in _qi217,
      '엔진이 자기 판정어를 되읽으면 표시가 계산이 된다')

# ── ⓑ 제목 — '최적'·'TOP'(순위) 주장을 걷어냈다 ─────────────────────
check("'최적 종목'이라 부르지 않는다", '최적 종목' not in _w217,
      '정밀분석은 상위 몇 개뿐이다 — 최적을 주장할 수 없다')
...
# ── ⓒ 비중 제안 — 추천 통과일 때만, 감출 땐 사유를 적는다 ────────────
check("비중 제안이 중앙 판정 recommended 에 걸린다",
      "if _pos_sug and CORE.get('recommended'):" in _w217,
      '새 조건을 만들지 않고 R186 과 같은 자를 쓴다 (§2-6)')
check("감출 때 조용히 지우지 않고 사유를 적는다 (§3)",
      '얼마나 살지는 살 만한 자리일' in _w217
      and 'elif _pos_sug:' in _w217)
# 심어서 잡히는지 — 조건 없는 옛 코드가 되살아나면 걸려야 한다 (§6)
check("조건 없는 옛 표기를 심으면 잡는다",
...
if not _node229:
    ...
    # ── 라운드 202 — **끝까지 도는 것**만으로는 모자란다 ──────────────
    #   201 이 스크립트를 되살리자 버튼이 눌리기 시작했고, 그러자 두
    #   가지가 더 틀려 있었다. 둘 다 스크립트는 멀쩡히 끝까지 돈다:
    #     ⓐ 지표를 **숨기기만** 해서 숨은 series 가 일봉 2,997개의 시각을
    #        그대로 쥐었다 → 메인 칸 시간축이 일봉인 채라 논리 586 이
    #        **2016년**을 가리켰다 (거래량 칸만 맞았다)
    #     ⓑ 지표를 끄며 생긴 리사이즈를 라이브러리가 **다음 프레임에**
    #        처리하면서 방금 넣은 범위를 덮어썼다 → 주봉 '1년' 이 5년치
    #   ⓐ 는 구조로 막는다 — 숨기는 것과 **비우는 것**은 다르다.
    _js202 = _c227
    check("봉을 바꿀 때 지표 데이터를 **비운다** (숨기기로 끝내지 않는다)",
          'SNAP.forEach' in _js202 and "setData(tf === 'D' ? orig : [])" in _js202,
          '숨은 series 도 시각을 쥐고 있어 시간축을 만든다')
    check("원본을 적어 두어 되돌아올 때 채운다",
          'const SNAP = new Map()' in _js202 and 'function remember(' in _js202)
    check("생성 자리가 전부 remember 를 지난다",
          _js202.count('remember(d.mk())') == 2,
          f"{_js202.count('remember(d.mk())')}곳 — 오버레이·보조패널 둘")
    check("범위를 다음 프레임에도 넣는다 (리사이즈가 덮어쓴다)",
...
print()
print("§240 R223 — 포트폴리오 '섹터별 평가' · 일곱 축 일정 (2026-09-04)")
print("-" * 72)
# ── 무엇이 있었나 ────────────────────────────────────────────────────────
#   사용자: "내 포트폴리오 섹터별로 좋은지 평가를 내리는 게 좋지 않을까?" —
#   화면은 업종별 비중·원장 적중·HHI·국면을 이미 세어 보여 준다. 없던 것은
#   그 숫자를 **어떻게 읽어야 하는지**다: 업종별 적중 차이(53~70%)는 표본·국면
#   조성 차이 안에 있어 업종을 고르는 근거가 못 된다(R46 기각 · R181 은 11-16
#   뒤 재측정). 그 말을 안 하면 사용자는 높은 업종을 고르라는 뜻으로 읽는다(§9).
...
# 첫 판은 잰 날짜를 글자로 박았다가 R78 의 잠금("어느 .py 에도 옛 재평가일이
# 박혀 있지 않다")에 걸렸다 — R159 의 실측일이 하필 옛 재평가일과 같은 날이다.
# 날짜·행수는 R159 산출물에서 읽는다. 손으로 적은 숫자가 둘 사라졌다.
check("면책 문구의 잰 날짜·당시 행수를 R159 산출물에서 읽는다 (손으로 적지 않는다 · §2)",
      "'data', 'census_r159.json'" in _w231
      and "{_c159_when}" in _w231
      and "_c159.get('ledger_rows')" in _w231
      and '184,759' not in _w231.split("census_r159.json")[-1][:1500])
check("ETF 의 업종 없음은 '구조상 없음'이지 '못 채운 것'이 아니다 (R218 · §3)",
      'ETF(업종 없음 · 구조상)' in _w231 and '종목을 열면 채워집니다' in _w231)
# ── 일정 문서 — 일곱 축 전부 · 날짜는 박제 파일과 같다 ─────────────────────
with open(_os.path.join(PROJ, 'docs', 'SCHEDULE_R223_AXES.md'), encoding='utf-8') as _f240:
    _sch240 = _f240.read()
import forward_eval as _fe240
check("일정 문서가 일곱 축을 전부 다룬다",
      all(k in _sch240 for k in ('`model`', '`scoring`', '`rulebook`', '`schema`',
                                 '`news`', '`valuation`', '`sector`')))
check("일정 문서의 전방 재평가일이 박제 파일의 값과 같다 (두 곳이 어긋나지 않게)",
...
print()
print("§252 R235 — 이동평균선 이름에 이동평균이 아닌 값이 들어 있었다 (2026-09-08)")
print("-" * 72)
# ── 무엇이 있었나 (사용자 지적 · 2026-09-08) ──────────────────────────────
#   시나리오 표가 **현재가 × (1 ± 변동성 배수)** 를 만들어 '20일선 복귀'·'60일선 지지'·
#   '60일선 종가 이탈'이라고 이름 붙였다. 실측(삼성전자 · 현재가 270,000원): 276,157 =
#   현재가×(1+vol×0.5) · 267,537 = ×(1−vol×0.2) · 263,843 = ×(1−vol×0.5). 같은 '60일선'이
#   두 값이었고 어느 것도 실제 이동평균이 아니었다(§3 — 못 읽은 것을 남의 값으로 만들지
#   않는다 · R164·R220 과 같은 모양). 곁들여 vol_20 은 ATR 이 아니라 20일 일간수익률
#   표준편차인데 변수 이름이 atr_val 이었고, 변동성 미수신 시 0.02 로 가격을 만들었으며,
#   횡보 '1차 목표'가 지지선보다 낮았고, 하락 행의 손절이 중앙 판정이 아닌 값이었다.
_sc252 = _w231.split("조건별 대응 시나리오 (변동성 기준 · 예측 아님)")[1].split(
    "[Section 1-6 Spec] 목표가격 5종 세트")[0]
check("변동성 배수 가격에 이동평균선 이름을 붙이지 않는다 (옛 세 문구가 사라졌다)",
      "20일선 복귀(`{curr_price*(1 + atr_val*0.5)" not in _w231
      and "60일선 지지(`{curr_price*(1 - atr_val*0.2)" not in _w231
      and "60일선 종가 이탈(`{curr_price*(1 - atr_val*0.5)" not in _w231)
check("이동평균선 이름에는 tech_df 의 실제 sma_20 · sma_60 을 읽어 넣는다",
      "_ma20_235, _ma60_235, _rsi235 = _last235('sma_20'), _last235('sma_60'), _last235('rsi_14')" in _sc252
      and "_ma235('20일선 회복', _ma20_235)" in _sc252
      and _sc252.count("_ma235('60일선") == 2)
check("같은 60일선은 두 행에서 같은 변수 하나를 읽는다 (값이 갈리지 않는다)",
      _sc252.count('_ma60_235') == 3 and 'sma_60' in _sc252)
check("이동평균이 없으면 '확인 불가' — 다른 값으로 채우지 않는다 (§3)",
      "return f\"{name} 확인 불가 (이동평균 미수신)\"" in _sc252
      and "return v if (v == v and v > 0) else None" in _sc252)
check("변동성 미수신이면 참고 가격대를 만들지 않는다 (0.02 기본값 경로 제거)",
      "if 'vol_20' in tech_df.columns else 0.02" not in _w231
      and "변동성 없이 시나리오 가격을 지어내지 않습니다" in _sc252)
check("변동성의 정체를 이름으로 적는다 — ATR 이 아니라 20일 일간수익률 표준편차",
      "20일 일간수익률 표준편차 {atr_val * 100:.2f}%" in _sc252
      and "평균 실제 범위(ATR)가 아니고" in _sc252)
check("범위를 20일 예측 구간이라 부르지 않는다 (제목·칸 이름)",
      "조건별 20일 대응 시나리오 대시보드" not in _w231
      and "20일 뒤 가격의 예측 구간도 아닙니다" in _sc252
      and "<th>변동성 배수 참고 가격대</th>" in _sc252
      and "<th>예상 가격 범위</th>" not in _sc252)
check("상승 1차 참고 목표선이 상승 확인선보다 낮으면 숫자를 내지 않는다 (정합성 게이트)",
      "_bull_ok235 = _bull_t1_235 > _bull_mark235" in _sc252
      and "참고 목표선이 상승 확인선보다 낮습니다 (정합성 미충족)" in _sc252)
check("횡보 행은 지지선보다 낮은 값을 '1차 목표'라 부르지 않는다",
      "검증된 횡보 진입계획이 없어 목표선을 내지 않습니다" in _sc252
      and "side_target" not in _sc252)
check("청산 기준은 중앙 판정에서 온다 — 이 표가 손절선을 새로 만들지 않는다 (§4)",
      "_stop_new235 = CORE.get('new_stop')" in _sc252
      and "_stop_hold235 = CORE.get('hold_stop')" in _sc252
      and "종가 {bear_target:,.0f}{unit_str} 이탈 즉시 손절" not in _w231)
check("판정하지 않는 수급 조건은 그렇다고 적는다 (충족으로 세지 않는다)",
      _sc252.count("이 화면에서 판정하지 않습니다 (수급 미연동)") == 2
      and "'리셋'의 기준은 정해져 있지 않습니다" in _sc252)
check("이 화면이 새 매수·매도 기준을 만들지 않는다고 말한다 (중앙 판정 우선)",
      "새로운 매수·매도" in _sc252 and "기준을 만들지 않습니다" in _sc252)
# 하락 행에서 그 선을 하회하면 시나리오가 **성립**한다 — '무효화'는 뜻이 뒤집힌다 (화면 실측에서 잡았다)
check("하락 행은 확인선을 무효화선이라 부르지 않는다",
      "이 시나리오는 하회하면 <b>성립</b>합니다" in _sc252
...
# ⑨ 재고 나면 화면 문구도 같이 바뀐다 — '아직 재지 않았다'가 남으면 거짓이다
#    라운드 248 이 쓴 그 문장을 라운드 249 가 재서 무효로 만들었다. 값·문턱·
