    ...
    try:
        ...
        if _fed182 and 'model' not in _ax_plan:
            _ax_plan['model'] = (
                ...
                f"(국면 라우팅 판정 · 반등 확인 재측정) 뒤 모델 규칙을 "
...
_NAV_SUB = [
    ...
    {'title': '1. 오늘의 시장', 'items': [
        ...
        # 라운드 290 — 어제 만든 절인데 갈 길이 없었다(라운드 105·124·136 과
        #   같은 자리). 본문 차례가 추천 → 개장 전 결론 → 간밤 시황이므로
        #   묶음 안에서도 그 차례다 (§170 ⓔ — 묶음 안은 본문 순서와 같아야 한다).
        {'key': 'market', 'label': '간밤·전일 시황', 'icon': 'chart',
         'href': '#nav-market'},
    ]},
    # 내 자산 (3 · 4)
    ...
    {'title': '4. 검증과 이력', 'items': [
        ...
        #   인지를 적어 둔 값이라(지난 사용자 결정을 순서 맞추기 편의로 안
        #   뒤집는다) **재고 나서 올리지 않는다.** 대신 이 절은 홈이 열리면 바로
        #   보이는 자리(첫 화면 맨 위)이고, 같은 것을 더 자세히 내는 '모델 성적'
        #   이 바로 아래 있다 — 갈 길이 없는 절이 아니다.
        ...
        if not _deferred228:
            ...
            if len(top_recs) == 0:
                ...
                st.caption(
                    ...
                    f"(2026-08-16 실측 · 차이 중앙값 0.0%p). 정밀분석하지 않은 "
...
if not _wl_body:
    ...
    st.info(
        ...
        "(2026-08-16 재확인 실측). 그래서 이 표는 **순위표가 아니라 "
        "가격 기준표**입니다.")
    ...
    # ⚠️ 라운드 292 — 사용자: *"보유중도 추가매수 가능이 제일 위로 올라가야하는거 아냐?"*
    #   세어 보니 맞는 말이었다(2026-09-14 · 보유 16행): 갈래가 **보유 유지 14 · 추가 매수
    #   가능 2** 인데, 종전 순서표는 '추가 매수 가능'을 '보유 유지' **뒤**에 두어 오늘
    #   **손댈 수 있는 두 행이 15·16번** — 맨 밑에 있었다. 아무것도 할 게 없는 14행이
    #   그 위를 다 차지했다. 라운드 287 이 '정리 검토'를 10번째에서 건져낸 것과 **같은
    #   결함의 거울상**이다.
    #   두 라운드를 함께 세우는 규칙은 하나다 — **손댈 수 있는 것이 손댈 것 없는 것보다
    #   위**, 그리고 손댈 것들 안에서는 **파는 쪽이 먼저**(늦으면 값을 잃는다 · R287).
    #   새 순위·새 문턱을 만들지 않았다 — 같은 표에서 한 칸을 옮겼을 뿐이다(§2-6).
    _WL_SELL_RANK = ('정리 검토', '일부 정리', '추가 매수 가능', '보유 유지',
                     '보유 기준 미산출')
    # 라운드 292 — '표본외 성적 미달'은 **기다려서 풀리는 칸이 아니므로** 대기들보다
    #   아래, 사실상 못 사는 칸들 옆에 둔다(살 자리가 가까운 순 · 새 문턱 없음).
    ...
    _wl_groups = [('보유 중', _wl_owned,
                   '매입가를 적은 종목 — 보유자 기준 · 지금 손댈 수 있는 순'
                   '(파는 쪽이 먼저 · 같은 자리는 이름순)', _WL_SELL_RANK,
                   '지금 손댈 수 있는 순'),
                  ('미보유', _wl_free,
    ...
    if _fill_missing:
        ...
        if _mrow327 or _clicked166:
            ...
            _msg166.empty()
            if _fail166:
                st.error("채우지 못한 종목 — 사유를 그대로 적습니다:  \n"
    ...
    if not _pf_held and not _pf_watch:
        ...
        _uk.chip_row(_chips226, theme=_theme,
                     title=f"보유 {len(_pf_held)}종목 · 지금 할 일 (손댈 수 있는 순)")
        ...
        # 라운드 292 — '표본외 성적 미달'은 이름에 '대기'가 없으므로 아래 기본값
        #   (`'brand' if '대기' in k`)이 안 걸린다. 대기가 아닌 칸의 색을 적어 준다.
        ...
        if _sec_perf226:
            ...
            _det226.append(
                ...
                f"업종 게이트는 실측에서 기각됐습니다. 업황 조정은 {_uk._esc(_fed223)} "
                f"전방 재평가 뒤 다시 잽니다. 이 표는 비중을 세어 보여 줄 뿐입니다.</p>")
        ...
        _det226.append(
            ...
            f"않았습니다</b> ({_uk._esc(_c159_when)} — {_uk._esc(_c159_verdict)}. "
            f"원장은 그 뒤 커졌고 결론은 2026-09-03 에 다시 확인했습니다). 평균 매수가는 "
        ...
        _uk.note(
            ...
            f"({_c159_when} — {_c159_verdict}). 무엇을 사라거나 비중을 정하라고 "
...
if _home_cal.get('total_cases'):
    ...
    # 라운드 290 — 이름 있는 절인데 **앵커가 없었다.** 메뉴가 못 갈 뿐 아니라,
    #   절 지도(앵커 사이 높이)가 이 절을 **앞 절에 얹는다** — 관심종목이 실제보다
    #   1,009px 커 보이던 까닭이다. 앵커는 절 머리 바로 앞에 둔다.
    st.markdown("<div id='nav-trust'></div>", unsafe_allow_html=True)
...
# 라운드 290 — 어제 이 절을 만들어 놓고 **갈 길을 안 냈다.** 사용자가 청한 것은
#   *"보기 좋게"* 였는데 메뉴에 없으면 찾아가는 길이 없다(라운드 105·124·136 이
#   세 번 같은 자리에서 고친 그 모양 — 화면에는 있는데 갈 길이 없다).
st.markdown("<div id='nav-market'></div>", unsafe_allow_html=True)
_uk.section("간밤·전일 시황", "국내·해외 지수 · 원자재 · 금리 · 환율 (전일 대비)",
            theme=_theme)
...
# ── 간밤·전일 시황 — 지수 타일 바로 아래 한 판 (라운드 288) ──────────────────
# 사용자: *"밤새 전체적인 주식이나 시황 정리 · 뭔일이 있었는지 · 어떻게 될 것
#   같은지 보기 좋게."* **새 자료를 만들지 않았다** — 유가·구리·금·미10년·
#   변동성지수는 2013-12-31 부터 이미 수집 중이었고(R248), 종목별 절 안에
#   묻혀 있던 그 판을 여기로 올렸을 뿐이다. 지수 타일(위) + 이 판 + 국면 한 줄이
#   '간밤에 무슨 일이 있었나' 의 답이다.
# ⚠️ *"어떻게 될 것 같은지"* 에는 **예측을 쓰지 않는다.** 화면이 정직하게 낼 수
#   있는 답은 이미 채택된 **국면 게이트**뿐이고, 지표 둘을 이어 붙여 이유를
#   만드는 순간 그것은 지어낸 값이다(§3 · R248 이 정한 그대로).
try:
    ...
    _err248 = ''
if _macro248:
    # 절 제목이 이미 '간밤·전일 시황' 이므로 같은 말을 두 번 적지 않는다.
    #   이 판이 **위 타일에 없는 것**을 이름으로 말한다 — 타일은 전일 대비 한 칸,
    #   이 판은 5일·20일 창과 원자재·금리 축이다.
    st.markdown("**지수 · 원자재 · 금리 · 환율 — 5일 · 20일 방향**")
...
# ⚠️ 라운드 289 — 사유는 **구분자로 시작한다.** 타일은 값 아래 부제를 그리므로
#   눈으로는 붙어 보이지만, 글자로 펴면 `산출 불가 유사사례 9건 …` 이 되어 값과
#   사유의 경계가 사라진다(화면을 글자로 읽는 쪽 — 검사·스크린리더 — 에서는
#   사유가 없는 것과 같아진다). 붙어 있음을 **글자에도** 남긴다.
# ⚠️ 라운드 295 — 사용자: *"이건 매번 산출 불가인데 도움이 되는거 맞아?"* 먼저 셌다
#   (2026-09-15 · 8종목): **3종목만 비었고** 5종목은 13·27·44·46·53건으로 값이 나온다.
#   '매번'은 아니다. 그런데 **비는 종목은 정확히 0건**이었다 — 9건이 아니라 0이다.
#   그런데도 화면은 *"10건이 필요합니다"* 라고만 적어, **조금만 더 모으면 되는 것처럼**
#   읽힌다. 0 은 '조금 모자란' 것이 아니라 **닮은 자리를 아예 못 찾은 것**이고 기다려서
#   열린다는 보장이 없다 — 라운드 128·292 가 '표본 대기'에서 고친 그 모양이다(§3).
_na_why199 = ("— 닮은 과거 자리를 **하나도** 못 찾았습니다(0건). 표본이 조금 모자란 "
              "것이 아니라 비교할 사례가 없는 것이라, 거래일이 쌓인다고 열린다는 "
              "보장이 없습니다"
              if _na_n199 == 0 else
              f"— 유사사례 {_na_n199}건 · 확률로 환산하려면 10건이 필요합니다"
              if isinstance(_na_n199, int) else
              "— 유사사례 표본을 만들지 못했습니다")
_na_all199 = all(_g.get(k) is None
                 for k in ('tp_first', 'sl_first', 'undecided'))
# ⚠️ 라운드 295 — 셋이 **모두** 비면 빈 타일 셋을 그리지 않는다. 사용자가 본 화면이
#   `산출 불가` 세 칸에 같은 사유 세 번이었다 — 같은 말을 세 번 하면 정보가 아니라
#   소음이다(라운드 233 이 빈 20일 타일 여섯을 문장 하나로 바꾼 그 선례).
#   값이 하나라도 있으면 종전대로 셋 다 그린다(합이 100% 라 같이 봐야 한다).
if not _na_all199:
    _uk.stat_tiles([
    ...
    # 사유는 **구분자로 시작한 채** 바로 뒤에 온다 — 값과 사유가 글자로도 붙어 있어야
    #   한다(라운드 293 이 세운 규칙 · §288 이 그것을 센다).
    st.markdown("**목표·손절 선도달 확률** — 산출 불가")
    st.caption(_md_safe(_na_why199))
if _na_all199:
    ...
    st.caption(_md_safe(
        ...
        # 라운드 295 — 0건일 때는 이 약속을 하지 않는다. 닮은 자리를 못 찾은 것이라
        #   거래일이 쌓인다고 열린다는 보장이 없다(기다리면 된다고 말해 놓고 영영 안
        #   풀리면 화면이 없는 길을 가리킨 것이다 · R128 이 적어 둔 그대로).
        + ("  \n거래일이 쌓여 유사사례가 10건을 넘으면 자동으로 채워집니다."
           if _na_n199 else
           "  \n0건은 표본이 모자란 것이 아니라 **닮은 자리를 못 찾은 것**이라, "
           "거래일이 쌓인다고 열린다고 약속할 수 없습니다.")))
if _blend59:
    st.caption(_md_safe(
...
try:
    ...
    _eng_rows.append((
        ...
        f'국면 라우팅은 {_fe.eval_date_ko()} 전방 검증 전 — '
        f'이번 판단에 미사용',
        '참고'))
    if _blend59:
        _eng_rows.append((
...
# ⚠️ 라운드 288 — 여기 있던 **시장 판**(유가·금·금리·환율)은 종목별 절 안에
#   있었는데 그 값들은 **종목과 무관한 시장 자료**다. 사용자: *"간밤 시황 정리를
#   보기 좋게."* 재료는 이미 다 있었고 **흩어져 있는 것**이 문제였다(R248 이 매크로
#   에서 한 것과 같은 모양) — 지수 타일 바로 아래 '1. 오늘의 시장' 으로 올렸다.
#   두 벌로 두지 않는다(§4). 값·문턱·판정 불변.
...
with _ctx_c1:
    ...
    if _glob:
        ...
        # 라운드 288 — 종전엔 *"위 '시장 판'"* 이라 적었는데 그 판을 '1. 오늘의 시장'
        #   으로 올려 더는 '위'가 아니다. 자리 대신 **이름**으로 가리킨다(자리로
        #   가리킨 문구는 옮기는 순간 거짓이 된다 · R235·R250 의 그 자리).
        # ⚠️ 라운드 292 — 하루 전 이 줄은 *"위 '시장 판'"* 이라는 **자리**로 가리키다가
...
with st.expander("4대 분리 점수 세부 산출 근거 및 실시간 정량 기여도 펼쳐보기"):
    ...
    #   여기서는 읽기만 한다 — 화면이 산식을 다시 적으면 문장만 낡는다(R236).
    _acp252 = four_scores.get('analysis_confidence_parts') or {}
    if _acp252:
        _acp_txt252 = ' + '.join(
            f"{k} {v[0]:.0f}점×{v[1] * 100:.0f}%" for k, v in _acp252.items())
        st.write(f"- **분석 신뢰도**: {_acp_txt252} = "
                 f"**{four_scores.get('analysis_confidence', 0)}점** "
                 f"(모델검증 = 표본외 전략 품질 점수 · 표본외 결과가 없으면 0점)\n")
    else:
...
with tab_val:
    ...
    # ── 라운드 359 — 이 적정가가 **무엇으로 섰는지** 한 줄 (표시 전용) ────────
    #   라운드 358 이 적자 종목에서 지어낸 양수 이익을 쓰는 모형 다섯을 무효로 돌렸다.
    #   그러면 남는 것이 자산 기반뿐일 수 있다 — 실측(2026-09-22 · 유니버스 앞 200종목에서
    #   고른 표본): 적자 34종목 **전부** 자산 모형만이고 **28종목은 유효 모형이 하나**뿐이다
    #   (흑자 대조 25종목은 전부 이익 모형을 포함하고 유효 모형 4~7종). 그런데 화면의 큰
    #   숫자는 여덟 모형을 교차검증한 값처럼 읽힌다(R239 의 그 계열).
    #   **새 문턱도 새 수도 없다** — 엔진이 이미 내는 `model_results` 를 **세기만** 한다
    #   (§4 · R312 의 "화면은 세기만 한다"). 가름은 `model_kinds` 한 곳이 정한다.
    try:
        import model_kinds as _mk359
        _split359 = _mk359.split((val_eval or {}).get('model_results'))
    except Exception:                                          # noqa: BLE001
        _split359 = None
    if _split359 and _split359['asset_only']:
        st.caption(
            f"이 적정가는 **자산 기반 모형 {len(_split359['asset'])}종**으로만 섰습니다 — "
