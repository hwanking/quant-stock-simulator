...
def build(four_scores, tech_df, price, verdict=None):
    ...
    # ⚠️ 라운드 183 — 문턱을 **+15% 에서 0% 로** 당겼다.
    #   종전에는 적정가를 넘어도 +15% 까지 추천했다. 블라인드 실측:
    #       적정가 이하        n 335 · 적중 59.4% · 원시수익 **+0.238%**  ← 유일한 양수
    #       적정가 소폭 초과    n  71 · 적중 53.5% · 원시수익 −0.890%
    #       적정가 초과(경고)   n  97 · 적중 54.6% · 원시수익 −0.652%
    #   **적정가를 넘는 순간 음수**다. 새 숫자를 만들지 않았다 — 구역 판정이
    #   이미 쓰는 경계(적정가 × 1.00)를 그대로 쓴다 (§2-6).
    #   ⚠️ 표본이 작다(71·97). 방향은 개발 구간과 같지만 그 사실을 밝힌다.
    #     이 변경은 **차단만** 하므로 완화가 아니다.
    ...
    #
    #   라운드 182 에서 나는 §3("못 잰 것으로 거르지 않는다")을 들어 고지만
    #   붙였다. **그 방어가 틀렸다.** 원장을 구역별로 재 보니(매수권 58+ ·
    #   판정완료), 개발 구간에서는 '판정 불가'가 가장 좋아 보이는데
    #   **블라인드에서 뒤집힌다**:
    #
    #     [blind] 손익분기 62.9% (라운드 168)      n     적중%   분기와   수익%
    #       적정가 이하                            335   59.4    −3.5   +0.238
    #       안전마진 확보                        3,074   53.6    −9.3   −0.956
    #       적정가 소폭 초과                        71   53.5    −9.4   −0.890
    #       적정가 초과(경고)                       97   54.6    −8.3   −0.652
    #       판정 불가                           1,536   52.9   −10.0   −1.237  ← 최악
    #
    #   **적정가를 못 낸 종목이 블라인드에서 가장 나쁘다**(n 1,536 · 충분).
    #   그리고 **적정가 이하만 유일하게 양수**다.
    #
    #   → 두 가지를 막는다. **차단만** 하고 통과를 만들지 않으므로 완화가
    #     아니다. 새 숫자도 만들지 않는다 — 구역 판정이 이미 쓰는 경계
    #     (적정가 × 1.00)를 그대로 재사용한다 (§2-6).
    #
    #   ⚠️ 다만 §3 은 지킨다 — **못 잰 것과 모델이 거부한 것을 가른다.**
    #     시세·재무를 못 받아 계산을 못 한 경우(UNCALCULATED 등)까지 막으면
    #     라운드 37(2,997종목 전멸)과 같은 실수가 된다.
    #     막는 것은 **모델이 값을 거부한 경우**(OUT_OF_DOMAIN)뿐이다.
    #
    #   ⚠️ 표본을 밝힌다 — '소폭 초과'·'경고'는 블라인드 n 71·97 로 작다.
    #     방향은 개발 구간과 같지만(둘 다 손익분기 아래) 그 사실을 숨기지
    #     않는다. 근거: docs/RESULT_R183_VALUE_GATE.md
