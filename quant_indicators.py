...
class QuantIndicatorsEngine:
    ...
    def evaluate_valuation_metric(self, tech_df, fund_df, symbol=None):
        ...
        # 쓰는 것 자체를 모델 리스크로 본다). 모델 적정가가 시장가격보다 70% 넘게
        # **아래**면, 그것은 '고평가'가 아니라 **가격의 대부분이 미래 성장 기대**
        # 라는 뜻이다 — 현재 이익·장부가로 만든 적정가는 숫자만 그럴듯한 오답이
        # 된다 (실사례: 레인보우로보틱스 현재가 433,000원에 적정가 10,501원,
        # PER 6,766배). 이때는 수축·보정할 것이 아니라 '산출 불가'가 맞다.
        #
        # ⚠️ 라운드 54 감사 — 처음에 이 게이트를 abs() 로 걸어 **양방향**으로
        # 만들었더니 전체 583종목 중 291종목(49.9%)이 미산출이 됐다. 표본
        # 120종목을 뜯어 보니 레인보우 방향(하방)은 16종목뿐이고 104종목은
        # **상방** — PER 3~7배 흑자 가치주에 모델이 +100~250% 저평가를 매긴
        # 경우였다. 그 종목들은 이익이 가격을 설명하는 종목이라 게이트의 논거
        # 자체가 성립하지 않고, 상방 극단은 바로 아래 윈저화(+48% 상한 로그
        # 수축)와 추정치 안정성 감점(c_stable)이라는 전용 처리기가 이미 있다.
        # 문턱(70%)은 그대로 두고 방향만 문서화된 의도대로 하방 전용으로 고친다.
        _ood_gap = float(self.FV_CONF.get('out_of_domain_gap_pct', 70.0))
        if raw_upside_pct < -_ood_gap:
            _ood_ctx = []
            try:
    ...
    def compute_four_separated_scores(self, symbol, tech_df, fund_df, sim_res, val_eval):
        ...
        #   변동성 1배(기준가 − 일변동성) — 그때 적은 수는 '실전 체결률 79.3% · 평균 3.4일' 이었는데
        #   라운드 344 가 산출물과 대 보니 79.3% 는 블라인드 280건 값이고 3.4일은 출처가 없었다.
        #   지금 수는 entry_facts 가 산출물에서 읽는다(전체 5,389건 · 78.3% · 3.05일 · 일봉 모의 체결).
        ...
        _fs_out = {
            ...
            'entry_pullback_basis': '기준가 − 일변동성 1배 · ' + __import__('entry_facts').line(),
