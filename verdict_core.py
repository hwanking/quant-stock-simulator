...
#:   (⚠️ 위 *"여기서 고치지 않는다"* 는 **라운드 191 그날의 결정**이다 —
#:    라운드 350 에서 사람이 바꿨다. 이 절 맨 아래를 보라. 원문은 둔다.)
#:
#: ■ 라운드 205 — 그 사전등록을 실행했다. **기각 · 0.36 유지.**
#:   (원장 187,435행 · 2026-09-02 · `scripts/cost_unify_r205.py` ·
#:    산출물 `data/cost_unify_r205.json`)
#:   R1  0.36 은 규칙집 항목의 **어떤 부분집합·자연 변형으로도 안
#:       나온다**(전 조합 {0.03,0.18,0.20,0.21,0.23,0.38,0.41} · 편도
#:       0.205 · 세금 제외 0.21 · 슬리피지 제외 0.23) — 산술 미아다.
#:       세금을 0.15 로 두면 0.03+0.15+0.18=0.36 이 정확히 나오지만
#:       세율 근거 문서가 저장소에 없다 — 해석이지 측정이 아니다.
#:   R2  슬리피지 0.18 도 세율 0.20 도 **측정된 적 없다** — 셋 다
#:       가정이다. PREREG_R54 의 내역(0.03×2+0.20+0.18)은 합이 0.44
#:       인데 0.36 이라 적은 또 다른 어긋난 사본이다.
#:   R3  블라인드 58+ (n=5,515) 순 EV 는 세 비용 모두 음수
#:       (−1.24 / −1.30 / −1.35) — 방향 동일. 통과.
#:   R4  **미달 — 그래서 기각이다.** 기대값 게이트 통과분의 블라인드
#:       원수익 평균이 게이트를 조일수록 **나빠진다**:
#:         0.30: 702건(블라인드 193) 적중 45.1% · −3.42%
#:         0.36: 270건(블라인드  70) 적중 44.3% · −4.07%
#:         0.41: 127건(블라인드  31) 적중 48.4% · −5.83%
#:       0.41 은 걸러 내는 게 아니라 표본만 줄인다. 블라인드 표본이
#:       작아(31~193) 확정은 아니나, "나빠지지 않는다"는 조건을 세울
#:       근거가 없다 — 사전등록대로 산술 정합보다 **성적이 우선**이다.
#:   덤: 통과분(−3.4~−5.8%)이 58+ 전체 평균(−0.94%)보다 나쁘다 —
#:       이 기대값 복원 게이트 자체가 블라인드에서 역선별일 수 있다.
#:       규칙 변경은 별도 사전등록 감이다. 여기 적어만 둔다.
#: ■ 라운드 350 — **값을 올렸다. 근거는 성적이 아니라 세율 사실이다**
#:   (2026-09-22 · 사용자 결정: *"거래비용 0.41로 바꿔줘."*)
#:   R205 의 R1 이 *"세금을 0.15 로 두면 0.03+0.15+0.18=0.36 이 정확히 나오지만
#:   세율 근거 문서가 저장소에 없다"* 고 적어 두었다. **그 근거가 생겼다** —
#:   2026-01-01 양도분부터 증권거래세가 코스피·코스닥 모두 **0.15% → 0.20%**
#:   다(금융투자소득세 폐지에 따른 세수 보전 · 언론 보도로 확인 · 법령 원문은
#:   안 열었다 · `docs/RESEARCH_DIGEST_2026-09-16.md` §1). 즉 0.36 은
#:   **2025년 세율로 만든 수**이고, 오늘의 법정 세율로 적으면 규칙집 항목 합
#:   그대로 0.03 + 0.20 + 0.18 = **0.41** 이다 — 새 수를 고른 것이 아니다(§2-6).
#:
#:   ⚠️ **R205 의 기각은 취소하지 않는다.** 그 기각의 근거는 성적(R4)이었고
#:   오늘도 그대로다 — 기대값 게이트를 조일수록 통과분의 블라인드 성적이
#:   나빠졌다. 바꾼 근거는 성적이 아니라 **비용 가정이 사실과 달랐다**는
#:   것이다. 성적이 좋아져서 바꾸는 것이 아니므로 **이득을 주장하지 않는다**(§9).
#:
#:   바꾸기 **전에** 센 대가(`_probe/r350_cost_impact.py` · 리포트 91개 ·
#:   후보 418개): 비용 차감 기대값이 양수인 후보 **1 → 0** · 중앙 −0.31 →
#:   **−0.36%** · 벽의 높이 0.31 → **0.36%p**. 추천 수는 이미 0 이라 안 바뀐다.
#:
#:   ⚠️ 전방 구간 한가운데서 바뀐다 — 11-16 재평가는 **2026-09-22** 로 앞뒤를
#:   갈라 센다. 버전 원장의 rulebook 축 effective_from 이 그 날짜다.
COST_PCT = 0.41         #: 규칙집 RULES_TRADING_COSTS 항목 합과 같다 (회귀가 값으로 잠근다)
...
#: ⚠️ 라운드 292 — 사용자가 화면의 '표본 대기' 를 짚었다. 라운드 128 이 그 자리의
#:   **사유**를 둘로 갈라 놓고(검증을 못 했다 / 검증을 했고 못 미쳤다) **이름은 둘 다
#:   '신뢰도·표본 확보 대기'** 로 두었다. 그래서 뒤엣것은 *"사례가 쌓인다고 풀리는 조건이
#:   아닙니다"* 라고 적으면서 이름은 **확보 대기**라고 말하는, 스스로 어긋나는 칸이었다 —
#:   바로 이 목록이 *"무엇을 기다리는지가 이름에 들어간다"* 고 적어 둔 그 규칙을 어긴다.
#:   기다릴 것이 없는 쪽에 **기다림이 아닌 이름**을 준다(§3 · R221 의 '불가' 와 같은 자리).
